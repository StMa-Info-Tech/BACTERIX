// js bact
var ListBact = [];
var ListVir = [];
var ListGraph = [];
var ListGraph2 = [];
var nb_bact_init = 50;
var nb_vir_init = 50;
var taille = 50;
var food = 3000;
var space = 200;
var Th_DeathBact = 75;
var Th_DeathVir = 75;
var Th_ReproBact = 15;
var Th_ReproVir = 15;
var Mort = false ;
var Augm = true;
var posix
var posigrek
var vy
var x
const ctx = document.getElementById('myChart2');
const ctx2 = document.getElementById('myChart');


function rdmint(max) {
  return Math.floor(Math.random() * max);
}

const pointImage = new Image(20, 20)
pointImage.src = './image/favicon.ico'

var chart1 = new Chart(ctx2, {
	type: 'scatter',
    data: {
		datasets: [{
        label: 'Scatter Dataset',
        data: [{
			    x: -10,
                y: 0
            }, {
                x: 0,
                y: 10
            }, {
                x: 10,
                y: 5
            }, {
                x: 0.5,
                y: 5.5
            }],
            backgroundColor: 'rgb(255, 99, 132)',
            pointHitRadius: 15
        },{
        label: 'Scatter Dataset 2',
        data: [{
			    x: -10,
                y: 0
            }, {
                x: 0,
                y: 10
            }, {
                x: 10,
                y: 5
            }, {
                x: 0.5,
                y: 5.5
            }],
            backgroundColor: 'rgb(255, 99, 132)',
            pointHitRadius: 15
        }],
      },
        options: {
        elements: {
            point: {
            pointStyle: pointImage
          }
        },
        scales: {
          x: {
            grid: {
              drawOnChartArea: false
            },
            type: 'linear',
            position: 'bottom'
          },
          y: {
            beginAtZero: true,
            grid: {
              drawOnChartArea: false
            }
          }
        },
		animation: false
      }
    });

var chart2 = new Chart(ctx, {
      type: 'line',
      data: {
        labels: [65, 59, 80, 81, 56, 55, 40],
        datasets: [{
          label: 'My First Dataset',
          data: [65, 59, 80, 81, 56, 55, 40],
          fill: false,
          borderColor: 'rgb(75, 192, 192)',
          tension: 0.1
        },{
          label: 'My Second Dataset',
          data: [65, 59, 80, 81, 56, 55, 40],
          fill: false,
          borderColor: 'rgb(75, 192, 192)',
          tension: 0.1
        }]
      },
      options: {
        scales: {
          y: {
            beginAtZero: true
          }
        },
		animation:false
      }
    });
	
function addData(chart, label, x, y, num) {
    if (!chart.data.datasets) {
        chart.data.datasets = [];
    }
    const labelIndex = chart.data.labels.indexOf(label);
    const numericX = Number(x);
    const numericY = Number(y);

    if (!isNaN(numericX) && !isNaN(numericY)) {
        const dataPoint = { x: numericX, y: numericY };

        if (labelIndex !== -1) {
			chart.data.labels.push(label);
            chart.data.datasets[num].data.push(dataPoint);
            ;
        } else {
            chart.data.labels.push(label);
            chart.data.datasets[num].data.push(dataPoint);
            ;
        }

        chart.update();
    } else {
        console.error('Invalid x or y coordinate. Please provide numerical values.');
    }
}



function removeData(chart) {
    // Remove all labels
    chart.data.labels = [];

    // Remove all data points from each dataset
    chart.data.datasets.forEach((dataset) => {
        dataset.data = [];
    });

    chart.update();
}


function NewBact(posx, posy, vie, mort) {
  const Bact = {
    posx: posx,
    posy: posy,
    vie: vie,
    mort: mort,
    deplace: function() {
		const dx= (-1)**rdmint(100);
		const dy= (-1)**rdmint(100);
		this.posx=this.posx+dx;
		this.posy=this.posy+dy;
    }
  };
  return Bact;
}
let nb_tour = 0;
let nb_max_tour = 30;
function Main(){
	if(nb_tour<nb_max_tour){
		console.log('taile :'+ListBact.length);
		nb_tour++
		console.log('tour :'+nb_tour);
		for(let j=0;j<ListBact.length;j++){
				ListBact[j].deplace();
				food=food-1
				Repro = rdmint(100);
				if(Repro<=Th_ReproBact && Augm == true && ListBact.length<space){
					x = ListBact[j].posx ;
					y = ListBact[j].posy ;
					ListBact.push(NewBact(x,y,0,0));
					food=food-5
				}
				else if(Augm == true && ListBact.length>=space){
					Augm=false
				}
		}
		/*
		for(let j=0;j<ListVir.length;j++){
				ListVir[j].deplace();
				food=food-1
				Repro = rdmint(100);
				if(Repro<=Th_ReproVir && Augm == true && ListVir.length<space){
					x = ListBact[j].posx ;
					y = ListBact[j].posy ;
					ListVir.push(NewBact(x,y,0,0));
					food=food-5
				}
				else if(Augm == true && ListVir.length>=space){
					Augm=false
				}
		}	
		*/
		removeData(chart1)
		for(let i=0; i<ListBact.length;i++){
			addData(chart1,i,ListBact[i].posx,ListBact[i].posy,0)
		}
		/*
		for(let i=0; i<ListVir.length;i++){
			addData(chart1,i,ListVir[i].posx,ListVir[i].posy,1)
		}
		ListGraph2.push(ListVir.length);
		*/
		ListGraph.push(ListBact.length);
		updateCourbe();
		if(food<=0 || Mort == true){
			Mort = true
			Death_Bact()
			//Death_Vir()
		}
		var t = setTimeout(function(){ Main() }, 1000)
	}
	else{console.log('fin')}
	
}
function Death_Bact(){
	for(let i=0;i<ListBact.length;i++){
		if(rdmint(100)>Th_DeathBact){
			Delete_Item = ListBact.splice(i,1);
		}
	}
}
function Death_Vir(){
	for(let i=0;i<ListVir.length;i++){
		if(rdmint(100)>Th_DeathVir){
			Delete_Item = ListVir.splice(i,1);
		}
	}
}
function updateCourbe(){
	removeData(chart2);
	for(let i=0;i<ListGraph.length;i++){
		addData(chart2, i, i, ListGraph[i],0);
	}
	/*
	for(let i=0;i<ListGraph2.length;i++){
		addData(chart2, i+'-', i, ListGraph2[i],1);
	}
	/*
	*/
}
function init(){
	removeData(chart1)
	for(let i=0;i<=nb_bact_init;i++){
		posix = rdmint(taille);
		posigrek = rdmint(taille);
		vy = rdmint(5);
		addData(chart1, i, posix, posigrek,0);
		let x = NewBact(posix,posigrek,vy,0)
		ListBact.push(x);
 	}
	for(let i=0;i<=nb_vir_init;i++){
		posix = rdmint(taille);
		posigrek = rdmint(taille);
		vy = rdmint(5);
		addData(chart1, i+'-', posix, posigrek,1);
		let x = NewBact(posix,posigrek,vy,0)
		ListVir.push(x);
	}
	removeData(chart2);
	addData(chart2, 1, 1, nb_bact_init,0);
	addData(chart2, 1, 1, nb_vir_init,1);
	ListGraph.push(nb_bact_init);
	ListGraph2.push(nb_vir_init);
	Main()
}
init()